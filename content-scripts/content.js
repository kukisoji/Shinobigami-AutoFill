(function () {
    ...
    for (const targetElement of ninpouNameElements) {
      ...
      if (targetElement) {
        ...
        if (elements.type.value == "装備") {
          ...
          //間合関連(拡視器の関係で特技決定後)
          let targetrange = result?.[result_col.range] ?? "";
  ...
  async function GabaCheckStart() {
    ...
    if (document.getElementById("base.race")?.value == RACE_NINJA) {
      ...
      let ZyoiRyuha = document.getElementById("base.upperstyle")?.value || "";
      let KaiRyuha = document.getElementById("base.substyle")?.value || "";
        ...
        for (const targetSkillElement of ninpouTargetSkillElements) {
          ...
          if (targetSkillElement) {
            ...
            if (
              ...
              GabaFlag = true;
              ErrMsg +=
                (nameValue || `忍法(${baseId})`) + // .name 要素があればその値、なければIDで表示
                "の指定特技がありません。\n";
            }
          }
  ...
  function zyoiRyuha_get() {
    let upperStyleValue = document.getElementById("base.upperstyle")?.value || "";
